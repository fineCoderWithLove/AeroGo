...
func newTestRouter() *aerogo.router {
	r := aerogo.newRouter()
	r.addRoute("GET", "/", nil)
...
func TestParsePattern(t *testing.T) {
	ok := reflect.DeepEqual(aerogo.parsePattern("/p/:name"), []string{"p", ":name"})
	ok = ok && reflect.DeepEqual(aerogo.parsePattern("/p/*"), []string{"p", "*"})
	ok = ok && reflect.DeepEqual(aerogo.parsePattern("/p/*name/*"), []string{"p", "*name"})
	if !ok {
